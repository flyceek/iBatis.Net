...
namespace IBatisNet.DataMapper.TypeHandlers.Nullables
{
	public sealed class NullableCharTypeHandler : BaseTypeHandler
	{
		...
		{
			char? c = (char?)parameterValue;
			if (c.HasValue)
			{
				dataParameter.Value = c.Value;
			}
			else
			{
		...
		public override object GetValueByName(ResultProperty mapping, IDataReader dataReader)
		{
			...
			{
				result = new char?(dataReader.GetString(ordinal)[0]);
			}
			return result;
		}
		...
		public override object GetValueByIndex(ResultProperty mapping, IDataReader dataReader)
		{
			...
			{
				result = new char?(dataReader.GetString(mapping.ColumnIndex)[0]);
			}
			return result;
		}
		...
		{
			return new char?(System.Convert.ToChar(s));
		}
	}
}
